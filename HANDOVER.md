...
| | 現在 | 名義 |
|---|---|---|
| microCMS | サカイさん（サービスID `lindo`） | **→ LINDO**（この手順で移す） |
| Cloudflare | 未作成 | **LINDO名義で作成**（DNS/配信/メールが乗るため） |
| ドメイン styledbylindo.com | ムームードメイン（GMOペパボ） | **LINDO**（登録者名を要確認） |
| ロリポップ（現サイト＋メール） | — | **LINDO**（契約者を要確認） |
| GitHubリポジトリ | `nvidia9875`（個人） | サカイ保持。**例外はここだけ** |

GitHubだけ個人持ちだが、中身はビルド用コードのみで、データ（microCMS）も配信先（Cloudflare）もLINDO側にあるため、最悪リポジトリを失っても作り直せる。将来必要ならOrganizationへ移管も可能。
...
| SPF | **無し**（`v=spf1` が存在しない）。移管後もメール送信を続けるなら別途検討 |

ネームサーバーを Cloudflare に向ける前に、**MX・TXTを含む既存レコードを完全に移すこと。**
実測した全レコードの一覧は `MTG-HANDOVER.md` の Part 3 にある。

> **⚠️ 現行サイトとメールは同じロリポップ契約に同居している。**
> 新サイト公開後に「もうロリポップは要らない」と解約すると**メールも同時に消える**。
> `contact@` を受信のみで使っているなら Cloudflare Email Routing（無料）へ移せるが、
> **送信もしている場合は解約すると送れなくなる。**
