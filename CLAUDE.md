...
Root `assets/` (tokens.css, reveal.js, 07-kinetic.js, 08-process.js, vendor/) belongs to tracks 1–2. The WP theme has its **own separate asset tree** at `wp-theme/lindo/assets/` (lindo.css, main.js, loader.js, hero-shatter.js, artist-modal.js, admin-gallery.js). Editing the wrong tree is an easy mistake — confirm which track you're touching.
...
- **Animations use `transform`/`opacity` only**, always honor `prefers-reduced-motion`, and heavy motion (concepts 07/08, hero-shatter) degrades to a static layout on mobile / reduced-motion / no-WebGL.
