...
4. **microCMS → static** (`wp-theme/preview/` + `.github/workflows/deploy.yml`) — **the live production direction.** Content lives in microCMS; GitHub Actions runs `preview/render.php` and deploys to GitHub Pages. **This is where active work happens.** See `HANDOVER-DEV.md` — the single authoritative technical handover doc. (`HANDOVER.md`, `GO-LIVE.md`, `STATUS.md`, `CUTOVER.md`, `TODO.md` and the `MTG-*.md` files are build-time work notes and are **stale**; each now carries a banner saying so. `HANDOVER-LINDO.md` is the client-facing operations guide.)
...
- **セクション番号は 01 About / 02 What We Do / 03 Works / 04 Artists / 05 Business Partner / Contact。** 紛らわしいので注意: **`section-artists.php` は 03 Works**（作品→モーダル）で、**04 Artists は `section-talents.php`**（写真→外部の公式サイト）。microCMS も同様に `artists` が 03、`talents` が 04。`talents` API が未作成なら `preview/talents-seed.php` の初期値（SugarNote）で描画され、画像は `preview/talents-img/` から `build-site.sh` が同梱する。API を作った時点で CMS 側が常に優先。
