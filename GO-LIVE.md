...
## 9. お問い合わせフォームの有効化 — **Web3Forms に決定（2026-08-10）**

**実装は完了済み。あとはアクセスキーを1つ設定するだけ。**

Cloudflare Workers は不採用。同一オリジンで動かすには NS を Cloudflare へ移す必要があり、
「A レコードだけ差し替えて MX に触らない」という安全策を捨てることになるため。
外部サービスなら DNS に一切触らずに済む。

無料枠は **250通/月**（Formspree は50通/月）。スパムも送信数を消費するので、
枠が薄いと**本物の問い合わせが落ちる**。ここが選定の決め手。

### 有効化の手順

- [ ] 9-1. https://web3forms.com/ でアクセスキーを取得
      **入力したメールアドレスがそのまま送信先**になる。アカウント作成は不要
      ⚠️ **ロリポップに依存しないアドレスを推奨**（解約するとフォームが死ぬため）
- [ ] 9-2. キーを登録（**Secret ではなく Variable**。HTMLに出るので秘密ではない）
      ```
      gh variable set WEB3FORMS_ACCESS_KEY --repo nvidia9875/lindo-web --body "<キー>"
      ```
- [ ] 9-3. ビルド → フォームが出ることを確認
      ```
      gh workflow run deploy.yml --repo nvidia9875/lindo-web
      ```
- [ ] 9-4. **実際に送信して受信できることを確認**（ここを飛ばさない）
- [ ] 9-5. 送信後に `/thanks.html` へ遷移することを確認
      ※ ドメイン切替前は `styledbylindo.com/thanks.html` へ飛ぶので旧サイトに着く。
        メールは届く。切替後に正しくなる

### 仕組み（重要）

**キーの有無でフォームとメール導線が自動で切り替わる。**

| `WEB3FORMS_ACCESS_KEY` | 表示 |
|---|---|
| 設定あり | フォーム（実際に送信される） |
| 未設定 | メール導線 |

「押しても何も起きないフォーム」が構造的に発生しない作りにしてある。
止めたくなったら `gh variable delete WEB3FORMS_ACCESS_KEY` でメール導線に戻る。

### hCaptcha を入れていない理由

web3forms.com の外部スクリプトが必要で、CSP の `script-src 'self'` と
「JSは自前ホスト」の方針に反する。ハニーポット（`botcheck`）のみにしてある。
